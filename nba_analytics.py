...
np.sqrt(np.square(a[0])+np.square(a[1]))

steph = df[df.PLAYER_NAME == 'Stephen Curry']
steph_distr = steph.SHOT_DISTANCE.apply(lambda x: 3 if x>26, 2 else)
steph_points = steph_distr* df.SHOTS_MADE[df.PLAYER_NAME = 'stephen curry']
...
for i in range(1,30):
        new_distribution.SHOT_DIST = new_distribution.SHOT_DISTANCE.apply(lambda x: x + (i/10))
        new_distribution['proj_make_pct'] = new_distribution.SHOT_DISTANCE.apply(lambda x: log_reg.predict_proba(np.array(x).reshape(-1,1))[0][1])
        new_distribution['EV'] = new_distribution.shot_pct*new_distribution.proj_make_pct*4
        points_per_shot = new_distribution.EV.sum()
        plot_data.append((25+i/10,points_per_shot))
